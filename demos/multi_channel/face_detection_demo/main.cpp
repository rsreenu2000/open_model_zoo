    ...
    auto drawStats = [&](const cv::Size winSize) {
        if (FLAGS_show_stats && !stats.empty()) {
            ...
            while (true) {
                ...
                cv::putText(windowImage, stats.substr(currPos, newPos - currPos), pos, cv::HersheyFonts::FONT_HERSHEY_COMPLEX, 0.4,  cv::Scalar(0, 0, 255), 1);
...
//  #ifdef USE_TBB
    ...
    cv::moveWindow("Detection results", 480, 329);
